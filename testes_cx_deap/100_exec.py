	...
	for i in range(2):
		...
		for process in range(NUM_PROCESSES):
		    ...
		    if pid:
		        ...
		        os.execlp('python', 'python', 'analise_operadores.py', "../../Dropbox/operadores-cf0?/CF0" + str(w+1) + "-selBest(cxOnePoint, mutShuffleIndexes).txt", str(0), str(11),str(26), str(k))# overlay program
		        assert False, 'error starting program'   
		        os._exit(0)
		    j += 1
		for i, childBlend in enumerate(childrenBlend):
