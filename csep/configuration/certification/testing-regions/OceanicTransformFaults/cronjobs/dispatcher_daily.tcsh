...
set logfile="$logdir"/oceanic_transform_faults_"$year-$month-$day-$hour$min$sec"


# Invoke Dispatcher for "today" (with 31 day delay for the testing date):
nohup python $CENTERCODE/src/generic/Dispatcher.py --year="$year" --month="$month" --day="$day" --forecastType=PolygonBased --configFile=$CSEP/testing-regions/OceanicTransformFaults/cronjobs/dispatcher_daily.init.xml --waitingPeriod=31 --enableForecastXMLTemplate --enableForecastMap --publishServer=csep-usc@northridge.usc.edu --publishDirectory=/var/www/html/csep/data/us/usc/oceanic_transform_faults --logFile="$logfile" >& "$logfile" &
