...
#
# Run daily Dispatcher for North Western Pacific testing region one day models generation and evaluation
#
...
set logfile="$logdir"/nw_daily_"$year-$month-$day-$hour$min$sec"


# Invoke Dispatcher for "today" (with 31 day delay for the testing date):
nohup python $CENTERCODE/src/generic/Dispatcher.py --year="$year" --month="$month" --day="$day" --configFile=$CSEP/testing-regions/NWPacific/cronjobs/dispatcher_daily.init.xml --waitingPeriod=31 --enableForecastXMLTemplate --enableForecastMap --publishServer=csep-usc@northridge.usc.edu --publishDirectory=/var/www/html/csep/data/us/usc/nwpacific --logFile="$logfile" >& "$logfile" &
