...
//        System.out.println("After second pass, (min, max) = (" + tauMinEstimate + ", " + tauMaxEstimate + ") and res = " + residual);
    ...
    public static double uniformSumCumulativeDensity(int n, double s) {
        ...
        // sum probability densities up to s, making steps of n / 10000
        double sum = 0;
...
//        System.out.println("C++ CDF = " + p);
    ...
    /**
     * Given the specified data, estimate the two negative binomial parameters, using Eqn's 26 & 27 from Kagan & Jackon 2000.
     *
     * @param data array of data for which we're interested in obtaining negative binomial parameter estimates
     * @return 2 element array of negative binomial parameter estimates of the form:
     *  [0] = tau
     *  [1] = upsilon
     */
    public static float[] negativeBinomialParameterEstimates(short[] data) {
        float mean = ArrayUtil.average(data);
        System.out.println("mean = " + mean);
        float variance = ArrayUtil.variance(data);
        System.out.println("variance = " + variance);
        float upsilon = mean / variance;
        float tau = (mean * upsilon) / (1f - upsilon);

        float[] results = {tau, upsilon};
        return results;
    }
