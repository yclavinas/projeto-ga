function relm_PaintLTestCumPlot(rRelmTest, sXLabelDescription, hAxes);
% function relm_PaintLTestCumPlot(rRelmTest, sXLabelDescription, hAxes)
% -----------------------------------------------------
% Creates the result plots of one of the RELM L-tests
%
% Input parameters:
%   rRelmTest		            Record of results from the RELM L-test
...
vObservedX = [rRelmTest.fLogLikelihood, rRelmTest.fLogLikelihood];
vObservedY = [0,1];
plot(sort(rRelmTest.vSimulation), vIndex, 'r', vObservedX, vObservedY, 'k', 'linewidth', 1);
...
vXLim = xlim;
vPatch1X = [vXLim(1) rRelmTest.fLogLikelihood rRelmTest.fLogLikelihood vXLim(1)];
vPatch2X = [rRelmTest.fLogLikelihood vXLim(2) vXLim(2) rRelmTest.fLogLikelihood];
patch(vPatch1X, vPatch1Y, [0.8 0.8 0.8], 'facealpha', 0.7);
patch(vPatch2X, vPatch2Y, [0.8 0.8 0.8], 'facealpha', 0.7);

% Add information
title(['\gamma = ' num2str(rRelmTest.fGamma)]);
