...
#
class RELMLikelihoodTest (RELMTest):
    ...
    class Result (RELMTest.Result):
        ...
        def __init__ (self, 
            ...
            self[RELMLikelihoodTest.Result.LogLikelihood] = 0

            # Compute gamma
            self[Gamma] = CSEPUtils.frequencyOfOccurrence(self[RELMLikelihoodTest.Result.LogLikelihood],
        ...
        def writeXML (self, 
            ...
            log_likelihood_node = xml.addElement(RELMLikelihoodTest.Result.LogLikelihood,
                                      test_node)
            log_likelihood_node.text = repr(self[RELMLikelihoodTest.Result.LogLikelihood])

            ### Create gamma elements
            gamma_node = xml.addElement(Gamma,
                                        test_node)
            gamma_node.text = '%.3f' %self[Gamma]
    ...
    #===========================================================================
    xml = RELMTest.XML('LTest', 
                       Result.LogLikelihood,
                       [RELMTest.Result.Name],
                       {Gamma : RELMTest.Result.SimulationData},
                       {Gamma : CSEPUtils.frequencyOfOccurrence},
    ...
    def __init__ (self, group):
        """ Initialization for RELMLikelihoodTest class."""
    ...
    def type (self):
        ...
        return RELMLikelihoodTest.Type
    ...
    def evaluate (self, 
        ...
        RELMTest.prepareCatalog(self)

        test_name = '%s-%s' %(RELMLikelihoodTest.Type,
                              EvaluationTest.FilePrefix)

        CSEPLogging.CSEPLogging.getLogger(RELMLikelihoodTest.__name__).info('%s for %s'
                                                                            %(test_name,
                                                                              forecast_name))
    ...
    def __invoke (self, 
        ...
        result = RELMLikelihoodTest.Result(log_likelihood,
                                           log_likelihoods_simulations,
                                           log_likelihood_modifications,
                                           np_true_likelihood)
