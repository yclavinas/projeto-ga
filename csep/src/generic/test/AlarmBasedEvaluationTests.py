"""
Module AlarmBasedEvaluationTests
"""
...
from CSEPLogging import CSEPLogging
 ...
 # provided files.
 #
class AlarmBasedEvaluationTests (CSEPTestCase):
    ...
    # XML elements to validate for the MASS test: if attribute is provided in the 
    ...
    def initialize (self):
        ...
        CSEP.Forecast.initialize(forecast_weights = False)
    ...
    def testNWPacificMASSTest(self):
        ...
        result_filename = 'KJMASSResultsNWPacific.xml'        

        self.__evaluationTest(model,
                              test_list,
                              result_filename,
                              'nw_pacific',
                              AlarmBasedEvaluationTests.__XMLElementsMASS,
    ...
    def testSWPacificMASSTest(self):
        ...
        # Reference file with test results
        result_filename = 'KJMASSResultsSWPacific.xml'        
    ...
    def testSWPacificROCTest(self):
        ...
        # Reference file with test results
        result_filename = 'KJROCResultsSWPacific.xml'        

        self.__evaluationTest(model,
                              test_list,
                              result_filename,
                              'sw_pacific',
                              AlarmBasedEvaluationTests.__XMLElementsROC,
                              OneYearModelPostProcess.Type,
                              post_process_args)
    ...
    def testNWPacificROCTest(self):
        ...
        # Reference file with test results
        result_filename = 'KJROCResultsNWPacific.xml'        
    ...
    def testRELMMainshockMASSTest(self):
        ...
        # Reference file with test results
        result_filename = 'EbelMASSResultsRELMMainshock.xml'        
    ...
    def testRELMMainshockROCTest(self):
        ...
        # Reference file with test results
        result_filename = 'EbelROCResultsRELMMainshock.xml'        
    ...
    def testRELMMainshockAftershockROCTest(self):
        ...
        # Reference file with test results
        result_filename = 'EbelROCResultsRELMMainshockAftershock.xml'        
    ...
    #----------------------------------------------------------------------------
    #
    # Run ROC test evaluation for California hybrid forecast, 
    # and validate the results.
    #
    def testCaliforniaHybridROCTest(self):
        """ Run ROC test for the California hybrid forecast and succeed."""

        # Setup test name
        CSEPTestCase.setTestName(self, 
                                 self.id())
        
        model = 'JANUSOneYear_11_1_2012-fromXML.dat'
        post_process_args = [datetime.datetime(2012, 11, 1), 
                             datetime.datetime(2013, 11, 1)]
        test_list = ROCTest.Type

        # Reference file with test results
        result_filename = 'JANUSROCResults.xml'        
    ...
    #----------------------------------------------------------------------------
    #
    # Run MASS test for the California hybrid forecast, and validate the results.
    #
    def testCaliforniaHybridMASSTest(self):
        """ Run MASS test for the California hybrid forecast and succeed."""

        # Setup test name
        CSEPTestCase.setTestName(self, 
                                 self.id())
        
        model = 'CONANOneYearMd3_11_1_2012.xml'
        post_process_args = [datetime.datetime(2012, 11, 1), 
                             datetime.datetime(2013, 11, 1)]
        test_list = MASSTest.Type

        # Reference file with test results
        result_filename = 'CONANMASSResults.xml'        
    ...
    def __evaluationTest(self, 
        ...
        shutil.copyfile(os.path.join(AlarmBasedEvaluationTests.__referenceDataDir,
                                     reference_dir,
                                     catalog_file),
                        os.path.join(CSEPTestCase.TestDirPath, catalog_file))    

        ### Path to the random seed file
        random_seed_path = os.path.join(AlarmBasedEvaluationTests.__referenceDataDir,
                                        random_seed)

        # Copy forecast group directory to the runtime test directory
        group_dir = "forecasts"
        shutil.copytree(os.path.join(AlarmBasedEvaluationTests.__referenceDataDir,
                                     reference_dir, 
                                     group_dir),
        ...
        reference_file = os.path.join(AlarmBasedEvaluationTests.__referenceDataDir,
                                      reference_dir, 
                                      'results',
                                      result_file)
        ...
        CSEPLogging.getLogger(AlarmBasedEvaluationTests.__name__).info("Comparing reference evaluation \
test file %s with generated evaluation test file %s..." \
        ...
        for each_tag, tag_attribute in xml_tags.iteritems():
           ...
           for each_ref_elem, each_test_elem in zip(ref_elements,
              ...
              CSEPLogging.getLogger(AlarmBasedEvaluationTests.__name__).info("Comparing %s \
